	...
	.controller('gamePlayerSelectController', [
		...
		function ($scope, $http, $location, $routeParams, socket, $timeout) {
			...
			$scope.resetPlayers = function () {
				$scope.settings.players.forEach(function (obj) {
					obj.position = 0;
					obj.selected = false;
					delete obj.isYours;
				});
			...
			if (!$scope.settings.players.length) {
				...
				$http.get(urls.fetchPlayerList).then(
					function (response) {
						...
						$scope.settings.players = response.data;
						$scope.resetPlayers();
					},
					function (error) {
						$scope.hasPlayersFetched = true;
			...
			socket.on('selection', function (data) {
				if (
					data.gameID === $scope.settings.yourGameID &&
					$scope.settings.players[data.index].selected !== data.selection
				) {
			...
			$scope.setGameSelectCountdown = function () {
				...
				(function loop() {
					...
					if ($scope.count > 0) {
						...
						if (
							playerCount.yours >= 1 &&
							playerCount.total >= $scope.settings.minimumSelectedPlayers
						) {
	...
	.controller('gamePlayController', [
		...
		function ($scope, $location, $timeout, $interval, socket) {
				...
				if (snakeIndex !== -1) {
					...
					$interval(
						function () {
							...
							i++;
							$scope.competitors[index].position = $scope.snakes[snakeIndex][i];
						},
						300,
						$scope.snakes[snakeIndex].length - 1
					).then(function () {
						$scope.isPlayerMoving = false;
						$scope.isSnakeBite = false;
						$scope.interactedPlayer = -1;
			...
			$scope.showWinner = function (index) {
				$scope.competitors.forEach(function (obj, i) {
			...
			$scope.movePlayer = function () {
				...
				console.log('player posn -> ' + source + ' dice -> ' + $scope.dice);	
				if (source + $scope.dice <= 99) {
					$interval(
						...
						console.log(
							'checking winner posn ->' + $scope.competitors[index].position + 'player -> ' + index
						);
